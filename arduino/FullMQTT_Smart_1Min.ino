/*
 * ============================================================================
 * FULL MQTT TEST - TẤT CẢ THIẾT BỊ + MQTT BROKER + SMART PUBLISHING
 * ============================================================================
 * 
 * Kết hợp:
...
const long publishInterval = 60000; // 🔥 1 PHÚT = 60000ms (thay vì 5 phút)
...
void setup() {
  ...
  Serial.println("║   🎯 SMART PUBLISHING ENABLED                 ║");
  Serial.println("╚════════════════════════════════════════════════╝\n");
...
bool shouldPublishSensors() {
  ...
  if (firstBoot) {
    ...
    lastCheckTime = currentMillis; // 🔥 Reset check time
    return true;
  }
  ...
  lastCheckTime = currentMillis; // 🔥 Update check time
  
  // Bây giờ mới đọc sensors để kiểm tra thay đổi
  // Đọc sensor values hiện tại
  // ✅ FIXED: Luôn lấy giá trị DHT22, không check error
  TempAndHumidity data = dht.getTempAndHumidity();
  float currentTemp = data.temperature;
  float currentHumidity = data.humidity;
  
  int gasRaw = analogRead(MQ2_PIN);
  float currentGasPPM = gasRaw; // ✅ RAW value
  ...
  if (currentMotionState != lastMotionState) {
    ...
    // Không return true ở đây, chỉ gửi PIR thôi
  }
...
void readAndPublishSensors() {
  ...
  // ✅ FIXED: LUÔN LUÔN publish, không check error để đảm bảo có data
  TempAndHumidity data = dht.getTempAndHumidity();
  int dhtStatus = dht.getStatus();
  
  Serial.printf("🔍 DHT22 Status: %d ", dhtStatus);
  if (dhtStatus == 0) {
    Serial.println("(OK)");
  } else {
    Serial.printf("(ERROR - but publishing anyway)\n");
  }
  ...
  Serial.printf("   DHT22: %.1f°C, %.1f%% (Status: %d)\n", 
                data.temperature, data.humidity, dhtStatus);
  
  // MQ-2 Gas (RAW value - chưa calibrate)
  int gasRaw = analogRead(MQ2_PIN);
  lastGasPPM = gasRaw; // ✅ Trả về RAW value thay vì giả định ppm
  publishSensorData(SENSOR_GAS_CODE, "gas", lastGasPPM);
  Serial.printf("   Gas: %d (RAW - uncalibrated)\n", (int)lastGasPPM);
  delay(100);
...
void reconnect_mqtt() {
  while (!client.connected()) {
    ...
    if (client.connect(DEVICE_ID, mqtt_user, mqtt_pass)) {
      ...
      Serial.println("   ✓ 7 Sensor ping topics (DHT22 split into 2)");
...
void mqtt_callback(char* topic, byte* payload, unsigned int length) {
  ...
  // ========================================
  // XỬ LÝ SERVO (2 chiếc) - HỖ TRỢ 2 FORMAT
  // ========================================
  ...
  // ========================================
  // XỬ LÝ MOTOR (2 chiếc) - HỖ TRỢ 2 FORMAT
  // ========================================
    ...
    controlMotorA(speed);
    publishDeviceState(DEVICE_MOTOR_A_CODE, speed, "speed"); // ✅ Publish {"speed":255,"state":true}
  }
    ...
    controlMotorB(speed);
    publishDeviceState(DEVICE_MOTOR_B_CODE, speed, "speed"); // ✅ Publish {"speed":255,"state":true}
  }
  
  // ========================================
  // XỬ LÝ RELAY (2 chiếc) - HỖ TRỢ 2 FORMAT
  // ========================================
...
void publishDeviceState(const char* deviceCode, int value, const char* valueKey) {
  ...
  StaticJsonDocument<128> doc;
  doc[valueKey] = value; // "angle" or "speed"
  doc["state"] = (value > 0);
  doc["timestamp"] = millis();
...
void controlMotorA(int speed) {
  speed = constrain(speed, 0, 255); // ✅ Chỉ cho phép 0-255 (không âm)
